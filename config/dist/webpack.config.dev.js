...
var _require = require('webpack-manifest-plugin'),
    WebpackManifestPlugin = _require.WebpackManifestPlugin;
...
var ForkTsCheckerWebpackPlugin = process.env.TSC_COMPILE_ON_ERROR === 'true' ? require('react-dev-utils/ForkTsCheckerWarningWebpackPlugin') : require('react-dev-utils/ForkTsCheckerWebpackPlugin');

var ReactRefreshWebpackPlugin = require('@pmmmwh/react-refresh-webpack-plugin');

var createEnvironmentHash = require('./webpack/persistentCache/createEnvironmentHash'); // Source maps are resource heavy and can cause out of memory issue for large source files.
...
var babelRuntimeEntry = require.resolve('babel-preset-react-app');
...
var useTypeScript = fs.existsSync(paths.appTsConfig); // Check if Tailwind config exists

var useTailwind = fs.existsSync(path.join(paths.appPath, 'tailwind.config.js')); // Get the path to the uncompiled service worker (if it exists).
  ...
  return {
    ...
    cache: {
      ...
      buildDependencies: {
        ...
        config: [__filename],
        tsconfig: [paths.appTsConfig, paths.appJsConfig].filter(function (f) {
